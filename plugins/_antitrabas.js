...
export async function before(m, {conn, isAdmin, isBotAdmin, usedPrefix}) {
  ...
  const fakemek = {'key': {'participant': '0@s.whatsapp.net', 'remoteJid': '0@s.whatsapp.net'}, 'message': {'groupInviteMessage': {'groupJid': '5493873605231-5493873687620@g.us', 'inviteCode': 'm', 'groupName': 'P', 'caption': 'NaufraZapp-MD', 'jpegThumbnail': null}}};
  if (chat.antiTraba && m.text.length > 5000) { // Cantidad máxima de caracteres aceptados en un mensaje//
    if (isAdmin) return conn.sendMessage(m.chat, {text: `El administrador @${m.sender.split('@')[0]} acaba de enviar un texto que contiene muchos caracteres. -_-`, mentions: [m.sender]}, {quoted: fakemek});
    ...
    } else if (!bot.restrict) return m.reply('?? Para realizar acciones de eliminacion, el creador tiene que encender el modo restringido..');
  }
  return !0;
}
