...
if (new Date - global.db.data.users[m.sender].crime < 3600000) return m.reply(`> ⓘ Regresa en: ${msToTime(date - new Date())}, para volver a robar.`)
let randow
...
if (media === 'text3') return m.reply(`> ⓘ *${pickRandom(global.robar)}*\n\n${diamond} Diamantes\n${money} 🐢 NzCoins`).catch(global.db.data.users[m.sender].limit += diamond).catch(global.db.data.users[m.sender].money += money)
if (media === 'text4') return m.reply(`> ⓘ ${pickRandom(global.robmal)}\n\n${diamond} Diamantes\n${money} 🐈 NzCoins`).catch(global.db.data.users[m.sender].limit -= diamante).catch(global.db.data.users[m.sender].money -= crime)
if (media === 'text5') return conn.reply(m.chat, `> ⓘ Robaste a: @${randow.split`@`[0]} la cantidad de: ${exp} XP`, m, {contextInfo: {mentionedJid: [randow]}}).catch(global.db.data.users[m.sender].exp += exp).catch(global.db.data.users[randow].exp -= crime) 
} catch (e) {
console.log(e)}}
handler.help = ['robar'];
handler.tags = ['xp'];
...
global.robar = ['Al robar y entregar la mitad a los mafiosos, obtienes:', 'Los mafiosos te regalan:'];
global.robmal = ['Perdiste, no lograste escapar, obtienes:', 'La policía te atrapo, pierdes:']
