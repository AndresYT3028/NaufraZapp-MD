...
export async function before(m) {
  ...
  if (!somematch(['0', '1', '2', '3', '4', '5'], m.text)) return this.sendMessage(m.chat, {text: `> ⓘ Solo se admite responder numeros del 1 al 5.\n\n${teks}`}, {quoted: aki.soal});
  const {server, frontaddr, session, signature, question, progression, step} = aki;
  if (step == '0' && m.text == '5') return m.reply('> ⓘ No hay las preguntas, esta es ka primera pregunta.');
  ...
  try {
    ...
    if (anu.status != '200') {
      aki.sesi = false;
      aki.soal = null;
      return m.reply('> ⓘ La sesion del juego ha caducado, el juego termina aqui.');
    }
    anu = anu.result;
    if (anu.name) {
      await this.sendMessage(m.chat, {image: {url: anu.image}, caption: `> ⓘ Tu personaje es ${anu.name}?\n_${anu.description}_`, mentions: [m.sender]}, {quoted: m});
      aki.sesi = false;
      aki.soal = null;
    } else {
    ...
    aki.sesi = false;
    aki.soal = null;
    m.reply('> ⓘ El juego de \`Akinator\` ha caido, vuelva mas tarde.');
  }
  return !0;
}
