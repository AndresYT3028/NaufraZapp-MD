...
const handler = async (m, {conn, isAdmin, isOwner, args, usedPrefix, command}) => {
  ...
  if (isClose === undefined) {
	  const caption = `
> ⓘ Para cerrar el grupo temporalmente, use estos comandos:

> *${usedPrefix + command}* \`open 1\`
> *${usedPrefix + command}* \`close 1\`

> ⓘ Para que el grupo cierre en una hora.
`;
    m.reply(caption);
	  throw false;
  }
  ...
  await conn.groupSettingUpdate(m.chat, isClose).then(async (_)=> {
	  m.reply(`> ⓘ Ahora el *_grupo ${isClose == 'announcement' ? 'cerrado' : 'abierto'} ${args[1] ? `durante *${clockString(timeoutset)}_*` : ''}`);
  });
  if (args[1]) {
	 setTimeout(async () => {
