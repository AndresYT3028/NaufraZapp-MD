...
const handler = async (m, {conn, args: [effect], text: txt, usedPrefix, command, name}) => {
  if (!effect) throw '> ⓘ Bienvenido al menu de logos, use los siguientes comandos, por ejemplo:\n> \`#logo 3d-deep-sea-metal Naufrago\`\n\n> ⓘ Si el comando mensiona haber faltado un texto mas, uselo de esta manera:\n> _#logo Wolf-Logo-Galaxy Naufra|Zapp_\n\n> Aqui tiene la lista de logos disponibles:\n\n> ❒ \`.logo\` ' + effects.map((v) => v.title).join('\n> ❒ \`.logo\` ');
  if (!effects.find((v) => (new RegExp(v.title, 'gi')).test(effect))) throw `> ⓘ El efecto de: \`${effect}\`, no existe en la lista de logos. Seleccione uno existente por favor.`;  
  ...
  const res = await maker(effectoSelect?.url, [...text]).catch(_ => { throw '> ⓘ Falta un texto para completar el logo, por favor realicelo.' })
   if (typeof res == 'number') throw res == -1 ? `> ⓘ El efecto de: \`${effect}\`, no existe en la lista de logos. Seleccione uno existente por favor.` : `> ⓘ Use el comando de esta manera: *${usedPrefix + command} ${effect} ${new Array(res).fill('texto').map((v, i) => v + (i ? i + 1 : '')).join('|')}*`;
  await conn.sendMessage(m.chat, {image: {url: res.image}, caption: `*𝚃𝙾𝙼𝙰 𝚃𝚄 𝙸𝙼𝙰𝙶𝙴𝙽 𝙿𝙴𝚁𝚂𝙾𝙽𝙰𝙻𝙸𝚉𝙰𝙳𝙰!!*\n*𝙴𝙵𝙴𝙲𝚃𝙾: ${effect}*`}, {quoted: m});  
};
handler.help = ['logos'];
handler.tags = ['nulis'];
handler.command = /^(logomenu|menulogos|crearlogos)$/i;
